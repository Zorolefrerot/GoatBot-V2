...
module.exports = { config: { name: "quizma", aliases: ["quizmanga", "qm"], role: "0", author: "Merdi Madimba", version: "4.0", description: "Quiz Manga multivers ou par manga spécifique", category: "🎌 Manga & Anime" },

onStart: async function ({ event, message }) { const threadID = event.threadID;
...
onChat: async function ({ event, message, usersData }) { const threadID = event.threadID; const session = activeSessions[threadID]; if (!session) return;
...
if (session.status === "choosingUniverse") {
  if (body === "1") {
    ...
    // extraire la liste des mangas dispo
    const mangas = [...new Set(questions.map(q => q.manga))];
...
// === CHOIX MANGA ===
if (session.status === "choosingManga") {
