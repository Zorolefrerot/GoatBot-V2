...
module.exports = {
  ...
  onStart: async function ({ event, message }) {
    ...
    await message.send(
      "🎌 **Bienvenue dans le Quiz protagonistes manga !**\n\n" +
  ...
  onChat: async function ({ event, message, usersData }) {
    ...
    if (session.status === "choosingQuestions" && !isNaN(body)) {
      ...
      const filePath = path.join(__dirname, "prota.json");
      ...
      await message.send(`✅ Le quiz démarre avec **${nbQuestions} questions** ! Bonne chance ❤️‍🩹🎌`);
      sendQuestion();
      return;
    }
